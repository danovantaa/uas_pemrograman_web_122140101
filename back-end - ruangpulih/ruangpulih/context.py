...
def extract_text_to_markdown(base_dir, output_file="context.md"):
    ...
    target_directories = [
        os.path.join(base_dir, "ruangpulih", "alembic"),
        os.path.join(base_dir, "ruangpulih", "models"),
        os.path.join(base_dir, "ruangpulih", "scripts"),
        os.path.join(base_dir, "ruangpulih", "views"),
        os.path.join(base_dir, "ruangpulih")
    ]

    ruangpulih_root = os.path.join(base_dir, "ruangpulih")
    if not os.path.isdir(ruangpulih_root):
        print(f"Error: Direktori '{ruangpulih_root}' tidak ditemukan.")
        return
    ...
    with open(output_path, 'w', encoding='utf-8') as outfile:
        ...
        print(f"Memproses file-file langsung di direktori: {ruangpulih_root}")
        for item in os.listdir(ruangpulih_root):
            item_path = os.path.join(ruangpulih_root, item)
